...
function updateMatchesList() {
    ...
    if (team1 && season && ctx.matches && ctx.matches[season]) {
        const matches = ctx.matches[season].filter(match => 
        ...
        if (matches.length > 0) {
            matches.forEach(match => {
                ...
                row.innerHTML = `
                    ...
                    <td>${match.home_team_goal_count} - ${match.away_team_goal_count}</td>
            ...
            matchesListElement.innerHTML = `
                <tr>
                    <td colspan="3">No matches found for ${team1} in ${season}.</td>
                </tr>`;
        }
    } else {
        matchesListElement.innerHTML = `
            <tr>
                <td colspan="3">Please select a team and season.</td>
            </tr>`;
    }
}
