...
function render_map() {
    ...
    ctx.data["clubs_cities"].forEach(d => {
        ...
        // Check if the city is in the right country
        let city_country = country_data.filter(e => e.City == d.City)[0];
        if (city_country && city_country.Country == "RightCountry") { // Replace "RightCountry" with the actual country name you want to filter by
