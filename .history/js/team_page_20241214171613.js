    ...
    if (season === "All Seasons") {
        for (const s in ctx.teams) {
            ctx.teams[s].forEach(teamData => {
                teams.add(teamData.team_name); // Use 'team_name' field from CSV
            });
        }
    } else {
        if (ctx.teams[season]) {
    ...
    function updateStats() {
        ...
        if (team1) {
            ...
            if (season === "All Seasons") {
                data1 = [];
                for (const s in ctx.teams) {
                    const teamData = ctx.teams[s].find(team => team.team_name === team1);
                    if (teamData) {
                        data1.push({ season: s, data: teamData });
                    }
                }
            } else if (ctx.teams[season]) {
                data1 = ctx.teams[season].find(team => team.team_name === team1);
            }
