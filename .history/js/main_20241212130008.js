...
function city_event(event, city_name) {
    ...
    const countryCode = ctx.data["map-focus"].features.find(f => f.properties.NAME.toLowerCase() === city_name)?.properties.CNTR_ID;
    if (countryCode && countriesWithGlow.includes(countryCode.slice(0, 2))) {
        const countryPath = d3.select(`#countries-area path[data-country-code="${countryCode.slice(0, 2)}"]`);
