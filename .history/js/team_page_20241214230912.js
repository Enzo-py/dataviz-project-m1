...
function updatePlayersList() {
    ...
    // Default to latest season if "All Seasons" is selected
    if (season === "All Seasons") {
        season = "2023-2024";
    }
    ...
    if (!ctx.players || !ctx.players[season]) {
        playersListElement.innerHTML = "<p>No player data available for this season.</p>";
        return;
    }

    // Filter players and handle possible undefined values
    const players = ctx.players[season]
        .filter(player => player && player.Current_Club === team1)
        .filter(player => player.minutes_played_overall !== undefined);

    // Sort players by minutes played
    players.sort((a, b) => (Number(b.minutes_played_overall) || 0) - (Number(a.minutes_played_overall) || 0));
