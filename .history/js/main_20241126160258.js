...
function render_map() {
    ...
    d3.select(".map svg").append("g")
        ...
        .append("image")
        .attr("xlink:href", d => {
            let club = ctx.data["clubs_cities"].find(c => c.City.toLowerCase() === d.properties.NAME.toLowerCase());
            if (club) {
                let club_logo = ctx.data["clubs_logo"].find(e => e.Club === club.Club);
                return club_logo ? club_logo.Logo_URL : 'default/path/to/default-icon.svg';
            }
            return 'default/path/to/default-icon.svg';
        })
