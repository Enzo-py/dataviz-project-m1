...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateStats() {
        ...
        if (team1) {
            ...
            currentData = data1; // Store the current team data
            updateCharts(currentData); // Update charts with the new data
...
let currentData = null; // Declare a global variable to store current team data

function switch_page(btn) {
    const page_number = btn.getAttribute("data-page");
    document.querySelectorAll(".page").forEach(page => {
        page.classList.remove("active");
    });
    document.querySelector(".page." + page_number).classList.add("active");

    // Update charts when switching to the performance page
    if (page_number === "p2" && currentData) {
