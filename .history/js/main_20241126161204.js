...
function render_map() {
    ...
    .each(function(d) {
        ...
        if (clubs.length > 1) {
            const radius = 10; // Radius for circular arrangement
            ...
            clubs.forEach((club, index) => {
                ...
                const x = radius * Math.cos(angle) - 5; // Adjust x position for each logo
                const y = radius * Math.sin(angle) - 5; // Adjust y position for each logo
                
                cityGroup.append("image")
/ => d.properties.NAME.slice(0, 1).toUpperCase() + d.properties.NAME.toLowerCase().slice(1))
