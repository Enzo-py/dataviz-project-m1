...
function createPerformanceChart(data) {
    ...
    const margin = { top: 30, right: 80, bottom: 30, left: 220 };  // Increased left and right margins
    const width = 700 - margin.left - margin.right;  // Increased total width
    const height = metrics.length * 40;  // Slightly increased height per bar
    ...
    processedMetrics.forEach(metric => {
        ...
        svg.append("text")
            ...
            .attr("x", x(0) - (x(metric.homeValue) - x(0)) - 15)  // More space from bars
        ...
        svg.append("text")
            .attr("class", "bar-label")
            .attr("x", x(metric.awayValue) + 15)  // More space from bars
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
