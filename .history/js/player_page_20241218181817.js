...
function getPlayerImageUrl(player_name) {
    ...
    const [forename, surname] = playerName.toLowerCase().split(" ");

    // Search for the player in the PlayerData array
    const player = data.PlayerData.find(
        p => p.Forename.toLowerCase() === forename && p.Surname.toLowerCase() === surname
    );

    // Return the ImageURL if the player is found
    return player ? player.ImageURL : "Player not found";
}
