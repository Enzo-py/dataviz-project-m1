...
function create_player_card(player_id, player_card) {
    ...
    // Move player's image to where the number was
    player_identity.append("img")
        .attr("src", player_image_url)
        .attr("alt", player_name)
        .attr("class", "player-image");

    // Add player's number below the image
    player_identity.append("span")
        .text(player_number)
        .style("color", ctx.selected_players[player_id])
        .attr("class", "player-number");

    // Adjust player_identity_data position
    player_identity_data = head_card.append("div").attr("class", "player-identity-data")
