    ...
    if (team1 && season && ctx.players && ctx.players[season]) {
        ...
        if (players.length > 0) {
            players.forEach(player => {
                ...
                // playerItem.textContent = `${player.player_name} - ${player.position}`;
        ...
        playersListElement.playersListElementinnerHTML = `<p>Please select a team and season.</p>`;
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateStats() {
        ...
        if (team1) {
            ...
            updatePlayersList();
            console.log(playersListElement)
