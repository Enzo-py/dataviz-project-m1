...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll("#toggle-league-average").forEach(button => {
        button.addEventListener("click", function() {
            const isShowingAverage = button.textContent.includes("Hide");
            button.textContent = isShowingAverage ? "Show League Average" : "Hide League Average";
            updateCharts(isShowingAverage ? data1 : calculateLeagueAverage());
        });
    });
...
function createGoalsDonutChart(data) {
    // Clear existing chart
    d3.select("#goals-donut-chart").html("");

    const width = 380; 
    const height = 380;
    const radius = Math.min(width, height) / 2;

    const svg = d3.select("#goals-donut-chart")
        .append("svg")
...
function createShotsDonutChart(data) {
    // Clear existing chart
    d3.select("#shots-donut-chart").html("");

    const width = 380; 
    const height = 380;
    const radius = Math.min(width, height) / 2;

    const svg = d3.select("#shots-donut-chart")
        .append("svg")
        .attr("width", width + 100)
        .attr("height", height + 100)
        .append("g")
