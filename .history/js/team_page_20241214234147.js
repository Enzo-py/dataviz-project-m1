...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateStats() {
        ...
        if (team1) {
            ...
            // Add this line to create the performance chart
...
function createPerformanceChart(data) {
    ...
    const metrics = [
        ...
        { label: "Total Goals", home: "total_goal_count_home", away: "total_goal_count_away" },
        ...
        { label: "Cards", home: "cards_total_home", away: "cards_total_away" },
    ...
    const maxValue = d3.max(metrics.flatMap(metric => 
        [parseFloat(data[metric.home]), parseFloat(data[metric.away])]
    ));
    ...
    metrics.forEach(metric => {
        const homeValue = parseFloat(data[metric.home]);
        const awayValue = parseFloat(data[metric.away]);

        // Home bar
        svg.append("rect")
            .attr("class", "bar-home")
            .attr("x", x(0) - x(homeValue) + x(0))
            .attr("y", y(metric.label))
            .attr("width", x(homeValue) - x(0))
        ...
        svg.append("text")
            ...
            .attr("x", x(0) - x(homeValue) + x(0) - 5)
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
            .attr("text-anchor", "end")
            .text(homeValue.toFixed(1));

        svg.append("text")
            .attr("class", "bar-label")
            .attr("x", x(awayValue) + 5)
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
            .text(awayValue.toFixed(1));
    });
