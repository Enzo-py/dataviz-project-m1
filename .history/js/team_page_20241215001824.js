...
function createPerformanceChart(data) {
    ...
    // Update margins to give more space for labels
    const margin = { top: 30, right: 60, bottom: 30, left: 220 };  // Increased left margin
    const width = 600 - margin.left - margin.right;
    const height = metrics.length * 45;  // Increased height per bar
    ...
    processedMetrics.forEach(metric => {
        ...
        svg.append("text")
            ...
            .attr("x", x(0) - (x(metric.homeValue) - x(0)) - 8)  // More space from bars
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
            .attr("text-anchor", "end")
            .style("font-size", "14px")  // Larger font
            .text(Math.round(metric.homeValue));  // Round to integer

        svg.append("text")
            .attr("class", "bar-label")
            .attr("x", x(metric.awayValue) + 8)  // More space from bars
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
    ...
    svg.append("g")
        ...
        .call(g => {
            ...
            g.selectAll(".tick text")
                ...
                .attr("x", -10);  // Move labels left
        });

    // Add legend
    const legend = svg.append("g")
        .attr("transform", `translate(-20,${height + 10})`);  // More space below chart
