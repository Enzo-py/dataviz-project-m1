...
function createPerformanceChart(data) {
    ...
    const margin = { top: 30, right: 150, bottom: 30, left: 220 };  // Increased right margin
    const width = 800 - margin.left - margin.right;  // Increased total width
    ...
    processedMetrics.forEach(metric => {
        ...
        // Update home label position
        svg.append("text")
            .attr("class", "bar-label")
            .attr("x", x(0) - (x(metric.homeValue) - x(0)) - 150)  // Increased space from bars
        ...
        // Update away label position
        svg.append("text")
            .attr("class", "bar-label")
            .attr("x", x(metric.awayValue) + 150)  // Increased space from bars
            .attr("y", y(metric.label) + y.bandwidth() / 2)
            .attr("dy", "0.35em")
            .style("font-size", "14px")
