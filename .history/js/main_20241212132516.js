...
function city_event(event, city_name) {
    ...
    const logoDatum = logo.datum();
    let countryCode;
    if (logoDatum) {
        countryCode = ctx.data["nutsrg"].features.find(f => d3.geoContains(f, logoDatum.geometry.coordinates))?.properties.CNTR_ID.slice(0, 2);
        console.log("City:", countryCode, city_name);
    }
    main.js:107 Uncaught ReferenceError: countryCode is not defined
    at city_event (main.js:107:5)
    at SVGPathElement.<anonymous> (main.js:235:45)
    at SVGPathElement.<anonymous> (d3.v7.min.js:2:20686)
