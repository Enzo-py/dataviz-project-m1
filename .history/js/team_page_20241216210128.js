...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Function to update the "View Players" link with the selected team
    function updateViewPlayersLink() {
        const team1 = document.getElementById("team1").value;
        const viewPlayersLink = document.getElementById("view-players-link");
        if (team1) {
            const encodedTeamName = encodeURIComponent(team1);
            viewPlayersLink.href = `team_comparison.html?club=${encodedTeamName}`;
        } else {
            viewPlayersLink.href = "team_comparison.html";
        }
    }

    // Update the link when the page loads
    updateViewPlayersLink();

    // Update the link whenever the team selection changes
