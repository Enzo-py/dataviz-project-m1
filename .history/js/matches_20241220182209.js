...
function generateScoreEvolutionChart(match) {
    ...
    const y = d3.scaleLinear().domain([0, d3.max([match.home_team_goal_count, match.away_team_goal_count])]).range([height, 0]);

    const xAxis = d3.axisBottom(x).tickFormat(d => `${d}'`);
    const yAxis = d3.axisLeft(y).ticks(d3.max([match.home_team_goal_count, match.away_team_goal_count]));
