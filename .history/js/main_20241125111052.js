...
function load_data() {
    const files = [
        ...
        "data/club_cities_countries.csv",
...
function render_map() {
    ...
    ctx.proj = d3.geoIdentity()
        ...
        // Filter cities to only include those in the associated countries
        const cityCountryMap = new Map(ctx.data["club_cities_countries"].map(d => [d.City.toLowerCase(), d.Country]));
        ctx.data["cities"].features = ctx.data["cities"].features.filter(d => {
            const cityName = d.properties.NAME.toLowerCase();
            const country = cityCountryMap.get(cityName);
            return country && wanted_countries.includes(country);
        });
