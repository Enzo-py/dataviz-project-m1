...
function updateTopPlayers() {
    ...
    // Load player images
    d3.json("data/img/soccer_wiki.json").then(data => {
        const images = Array.isArray(data) ? data : JSON.parse(data);
        const imageMap = {};
        images.forEach(img => {
            const fullName = `${img.Forename} ${img.Surname}`;
            imageMap[fullName] = img.ImageURL || "data/img/default_player.png";
        });
    ...
    }).catch(error => console.error("Error loading player images:", error));
}
