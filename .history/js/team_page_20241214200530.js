...
function updateMatchesList() {
    ...
    if (team1 && season && ctx.matches && ctx.matches[season]) {
        const matches = ctx.matches[season].filter(match => match.home_team_name === team1 || match.home_team_name === team1);
        ...
        if (matches.length > 0) {
            matches.forEach(match => {
                ...
                matchItem.textContent = `${match.date} - ${match.home_team} vs ${match.away_team} - ${match.home_score}:${match.away_score}`;
