...
function getPlayerImageUrl(player_name) {
    // Assuming soccer_wiki.json is already loaded into a variable called soccerWikiData
    const playerData = soccerWikiData.PlayerData;

    // Split the player_name into forename and surname
    const [forename, surname] = player_name.split(' ');

    // Find the player in the PlayerData array
    const player = playerData.find(p => p.Forename === forename && p.Surname === surname);

    // Return the ImageURL if the player is found, otherwise return null
    return player ? player.ImageURL : null;
}

// Example usage
const player_name = "Nuno Mendes";
const imageUrl = getPlayerImageUrl(player_name);
