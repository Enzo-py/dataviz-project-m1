    ...
    function updateStats() {
        ...
        if (team1) {
            ...
            if (data1 && data1.length > 0) {
                // Display statistics for all seasons
                document.getElementById("team1-stats").innerHTML = data1.map(item => {
                    return `<h3>${team1} Stats (${item.season})</h3><p>${JSON.stringify(item.data)}</p>`;
                }).join('');
            } else if (data1) {
                // Display statistics for selected season
            ...
            // Update League
            const country = data1 && data1.length > 0 ? data1[0].data.country.toLowerCase() : data1 ? data1.country.toLowerCase() : "";
            const league = countryToLeague[country] || "Unknown League";
            teamLeagueElement.textContent = league;
            // Update Rank
            const rank = data1 && data1.length > 0 ? data1[0].data.league_position : data1 ? data1.rank : "Unknown Rank";
            teamRankElement.textContent = "Rank: " + rank;
            // Update Team Stats
            const wins = data1 && data1.length > 0 ? data1[0].data.league_position : data1 ? data1.rank : "Unknown Rank";
            const losses = data1 && data1.length > 0 ? data1[0].data.league_position : data1 ? data1.rank : "Unknown Rank";
            const draws = data1 && data1.length > 0 ? data1[0].data.league_position : data1 ? data1.rank : "Unknown Rank";
        } else {
